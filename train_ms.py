...
from data_utils import (
    ...
    load_filepaths_and_text_nepali     # <-- ADDED
)
from models import (
    SynthesizerTrn,
...
def run(rank, n_gpus, hps):
    ...
    # -------------------------------------------------------------
    # ✅ FIXED — Load training filelist correctly
    # -------------------------------------------------------------
    train_list = load_filepaths_and_text_nepali(hps.data.training_files)
    train_dataset = TextAudioSpeakerLoader(train_list, hps.data)
    ...
    if rank == 0:
        # -------------------------------------------------------------
        # ✅ FIXED — Load validation filelist correctly
        # -------------------------------------------------------------
        eval_list = load_filepaths_and_text_nepali(hps.data.validation_files)
        eval_dataset = TextAudioSpeakerLoader(eval_list, hps.data)
...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    image_dict = {
        "gen/mel": utils.plot_spectrogram_to_numpy(y_hat_mel[0].detach().cpu().numpy())
    }
